...
/**
 * @title UseNewTicket
 * @dev Script to use a ticket from the new 30-second event and test POAP minting
 */
contract UseNewTicket is Script {
    ...
    function run() external {
        ...
        console.log("Using Ticket from New 30-Second Event for POAP Integration Test");
        console.log("=============================================================");
        console.log("");

        // New 30-Second POAP Event address
        address eventAddress = 0x4e373bb270A9c5f79E2149075D23456C013fEb22; // New 30-Second POAP Event
        
        if (eventAddress == address(0)) {
            console.log("ERROR: Please update the eventAddress in this script");
            console.log("Run CreateNew30SecEvent.s.sol first to get the new event address");
            return;
        }
        ...
        // Step 1: Use the ticket (this should trigger POAP minting)
        console.log("Step 1: Using Ticket to Trigger POAP Minting...");
        
        // Try with ticket ID 1 first
        uint256 ticketId = 1;
        ...
        try testEvent.useTicket(ticketId) {
            ...
            try testEvent.useTicket(2) {
                ...
                console.log("   Trying with ticket ID 3...");
                
                // Try with ticket ID 3
                try testEvent.useTicket(3) {
                    console.log("   [SUCCESS] Ticket used successfully!");
                    console.log("   POAP should be automatically minted");
                    ticketId = 3;
                } catch Error(string memory reason3) {
                    console.log("   [ERROR] Ticket usage failed again:", reason3);
                    return;
                }
            }
        }
        ...
        // Step 2: Verify POAP minting
        console.log("Step 2: Verifying POAP Minting...");
        
        // Check if POAP was minted (event ID 2 for the new 30-second test)
        bool hasClaimed = poapContract.hasUserClaimedPOAP(2, msg.sender);
        if (hasClaimed) {
        ...
        // Step 3: Show POAP event details
        console.log("Step 3: POAP Event Details...");
        (uint256 eventId, string memory name, string memory description, string memory imageURI, string memory poapURI, bool isActive, address eventContract, address organizer, uint256 minted, uint256 createdAt) = poapContract.getPOAPEvent(2);
